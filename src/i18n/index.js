...
export const intlLocale = computed(() =>
  localeRef.value === 'en' ? 'en-US' : 'es-CO'
)
...
const initial = (() => {
  ...
  const nav =
    (typeof navigator !== 'undefined' && navigator.language || 'es').toLowerCase()
...
export const formatCurrencyCompact = (
  ...
  const numStr = new Intl.NumberFormat(loc, {
    minimumFractionDigits: 0,
    maximumFractionDigits: 2
  }).format(reduced)
...
export const formatDate = (
  date,
  options = { dateStyle: 'medium', timeZone: 'America/Bogota' }
) => {
  const loc = intlLocale.value
  const key = loc + '|' + nfKey(options)
