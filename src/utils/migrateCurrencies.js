export const migrateCurrencies = async uid => {
  ...
  try {
    ...
    for (const d of accSnap.docs) {
      ...
      if (!data.currency) {
        await updateDoc(doc(db, 'users', uid, 'accounts', d.id), { currency: defaultCode })
      }
    }
        ...
        if (colName === 'transactions') {
          if (data.isTransfer) {
            ...
            if (data.type === 'transfer-out' || data.type === 'transfer-in') {
              if (!data.currencyFrom && data.fromAccountId) {
                patch.currencyFrom = accountMap[data.fromAccountId] || defaultCode
              }
              if (!data.currencyTo && data.toAccountId) {
                patch.currencyTo = accountMap[data.toAccountId] || patch.currencyFrom || defaultCode
              }
            }
            if (Object.keys(patch).length) {
              await updateDoc(doc(db, 'users', uid, colName, d.id), patch)
            }
          ...
          if (!data.currency) {
            await updateDoc(doc(db, 'users', uid, colName, d.id), { currency: defaultCode })
          }
        }
      }
    }
