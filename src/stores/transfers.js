...
export const useTransfersStore = defineStore('transfers', () => {
  ...
  const init = async () => {
    ...
    try {
      ...
      }, e => { error.value = e?.message || 'Error'; status.value = 'error' })
    } catch (e) {
  ...
  const add = async payload => { try { const id = await createTransfer(payload); success(t('transfers.created')); return id } catch (e) { notifyError(t('errors.generic')); throw e } }
  const edit = async (transferId, patch) => { try { await updateTransfer(transferId, patch); success(t('transfers.updated')) } catch (e) { notifyError(t('errors.generic')); throw e } }
  const remove = async transferId => { try { await deleteTransfer(transferId); success(t('transfers.deleted')) } catch (e) { notifyError(t('errors.generic')); throw e } }
  const setFilters = f => { filters.value = Object.fromEntries(Object.entries(f || {}).filter(([_, v]) => v !== '' && v != null)) }
  ...
  return { items, status, error, selected, filters, init, dispose, add, edit, remove, setFilters, hasItems, byId, filtered }
})
