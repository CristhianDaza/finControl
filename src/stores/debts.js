...
export const useDebtsStore = defineStore('debts', () => {
  ...
  const subscribeMyDebts = async () => {
    ...
    try {
      _unsubscribe.value = subscribeDebts(list => { items.value = list; status.value = 'success' })
    } catch (e) {
  ...
  const update = async (id, patch) => {
    try { await updateDebt(id, patch); success(t('debts.notifications.updated')) }
    catch (e) { notifyError(e?.message || 'Error'); throw e }
  }

  const remove = async (id) => {
    try { await deleteDebt(id); success(t('debts.notifications.deleted')) }
    catch (e) { throw e }
  }
