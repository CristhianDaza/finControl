          ...
          export const useAuthStore = defineStore('auth', {
            ...
            getters: {
              ...
              isAdmin: s => {
                if (!s._cachedEnvAdmins) {
                  s._cachedEnvAdmins = (import.meta.env.VITE_ADMIN_UIDS || '')
                    .split(',')
                    .map(v => v.trim())
                    .filter(Boolean)
                }
                return !!s.profile && (
                  ...
                  s._cachedEnvAdmins.includes(s.user?.uid || '')
                )
              },
              isReadOnly: s => {
                const p = s.profile
                ...
                const expMs = p.planExpiresAt?.toMillis
                  ? p.planExpiresAt.toMillis()
                  : p.planExpiresAt
            ...
            actions: {
              ...
              async fetchUserProfile() {
                ...
                const snap = await getDoc(ref)
                if (snap.exists()) {
                  this.profile = { id: snap.id, ...snap.data() }
                } else {
                  this.profile = null
                }
              },
              ...
              async signup(email, password, code, confirmPassword) {
                ...
                if (password !== confirmPassword) {
                  this.error = t('errors.signup.password_mismatch')
                  return false
                }
                this.status = 'loading'
                try {
                  ...
                  if (!snap.exists()) {
                    this.error = t('errors.invite.not_found')
                    this.status = 'unauthenticated'
                    return false
                  }
                  const data = snap.data()
                  const now = Date.now()
                  const exp1 = data.expiresAt?.toMillis
                  ...
                  if (data.status !== 'unused') {
                    this.error = t('errors.invite.used')
                    this.status = 'unauthenticated'
                    return false
                  }
                  if (now >= Math.min(exp1 || now - 1, exp2 || now - 1)) {
                    this.error = t('errors.invite.expired')
                    this.status = 'unauthenticated'
                    return false
                  }
                  ...
                  await runTransaction(db, async (trx) => {
                    ...
                    const expA = fd.expiresAt?.toMillis
                      ? fd.expiresAt.toMillis()
                      : fd.expiresAt
                    const expB = fd.graceExpiresAt?.toMillis
                      ? fd.graceExpiresAt.toMillis()
                      : fd.graceExpiresAt
                    ...
                    const planExpiresAt = Timestamp.fromMillis(
                      (() => {
                    ...
                    trx.set(doc(db, 'users', cred.user.uid), {
                  ...
                  if (msg === 'used') {
                    ...
                    this.error = t('errors.invite.invalid')
                  }
                  this.status = 'unauthenticated'
                  return false
                }
              ...
              async logout() {
                ...
                try {
                  ...
                  try {
                    useSettingsStore().clearCacheOnLogout()
                  } catch {}
                  this.user = null
                  this.profile = null
                  this.status = 'unauthenticated'
              ...
              async applyCodeAndRefresh(raw) {
                ...
                try {
                  ...
                  const m = (e && e.message) || ''
                  const map = {
                    not_found: t('errors.invite.not_found'),
                    used: t('errors.invite.used'),
                    expired: t('errors.invite.expired'),
                    invalid: t('errors.codeInvalid'),
                  }
                  return { ok: false, error: map[m] || t('errors.codeInvalid') }
                }
              },
              async redeemCode() {
                return { ok: false, error: t('errors.codeInvalid') }
              },
            },
          })
