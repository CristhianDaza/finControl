...
export const useAdmin = () => {
  ...
  const countUsers = async () => {
    ...
    return {
      ...
      inactive: totalSnap.data().count - activeSnap.data().count
    }
  }
  ...
  const createInviteCode = async ({ plan, createdBy }) => {
    ...
    for (let i = 0; i < 5; i++) {
      code = genCode()
      const chk = await getDoc(doc(db, 'inviteCodes', code))
      if (!chk.exists()) break
    }
    const now = Date.now()
    const days =
  ...
  const invalidateInviteCode = async id => {
    if (gate()) return
    await updateDoc(doc(db, 'inviteCodes', id), { status: 'expired' })
  }
  ...
  const listInviteCodes = async ({
    ...
    for (const d of snap.docs) {
      ...
      if (data.usedBy) {
        try {
          const uSnap = await getDoc(doc(db, 'users', data.usedBy))
          if (uSnap.exists()) usedByEmail = uSnap.data().email || null
        } catch {}
      }
      items.push({ id: d.id, ...data, usedByEmail })
    }
    return items
  }
  ...
  const validateInviteCode = async code => {
    ...
    const snap = await getDoc(doc(db, 'inviteCodes', up))
    if (!snap.exists())
      return { ok: false, error: t('errors.invite.not_found') }
    const data = snap.data()
    if (data.status !== 'unused')
      return { ok: false, error: t('errors.invite.used') }
    const now = Date.now()
    ...
    const minExp = Math.min(
      exp1 ?? Number.POSITIVE_INFINITY,
      exp2 ?? Number.POSITIVE_INFINITY
    )
    if (minExp !== Number.POSITIVE_INFINITY && now >= minExp)
      return { ok: false, error: t('errors.invite.expired') }
    return { ok: true, doc: { id: snap.id, ...data } }
  }

  const migrateInviteCodes = async () => {
    if (gate()) return { created: 0, skipped: 0 }
