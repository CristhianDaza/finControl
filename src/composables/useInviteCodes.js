...
export const useInviteCodes = () => {
  const getCode = async code => {
    ...
    if (data.status !== 'unused') throw new Error('used')
    if (now >= Math.min(exp1 || 0, exp2 || 0)) throw new Error('expired')
    return { id: snap.id, ...data }
  }

  const applyCode = async (code, uid, email) => {
    const codeId = code.trim().toUpperCase()
    if (!codeId) throw new Error('invalid')
    ...
    return await runTransaction(db, async trx => {
      ...
      if (!userSnap.exists()) throw new Error('user')
      ...
      if (cd.status !== 'unused') throw new Error('used')
      if (now >= Math.min(exp1 || 0, exp2 || 0)) throw new Error('expired')
      const userData = userSnap.data()
      const curExpMs = userData.planExpiresAt?.toMillis ? userData.planExpiresAt.toMillis() : userData.planExpiresAt
      const base = (curExpMs && curExpMs > now) ? curExpMs : now
      const months = planMonths(cd.plan || 'monthly')
      const newExpMs = addMonths(base, months)
      const newExp = Timestamp.fromMillis(newExpMs)
      trx.update(userRef, {
        isActive: true,
      ...
      trx.update(codeRef, {
        status: 'used',
        usedBy: uid,
        usedByEmail: (email || userData.email || '').toLowerCase(),
