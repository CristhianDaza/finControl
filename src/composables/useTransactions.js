...
export const useTransactions = () => {
  ...
  const buildQuery = (opts = {}) => {
    ...
    const orders = (opts.orderBy && Array.isArray(opts.orderBy) && opts.orderBy.length)
      ? opts.orderBy
      : [
          { field: 'date', dir: 'desc' },
  ...
  const createTransaction = async payload => {
    ...
    await runTransaction(db, async trx => {
      ...
      if (type === 'income') {
        ...
        trx.update(debtRef, {
          remainingAmount: fromCents(nextRemC),
          status: nextRemC === 0 ? 'paid' : 'active',
  ...
  const updateTransaction = async (id, payload) => {
    ...
    await runTransaction(db, async trx => {
      ...
      trx.update(prevAccRef, {
        balance: fromCents(prevNextBalC),
        updatedAt: serverTimestamp()
      })
      ...
      const nextAccSnap = prev.accountId === newAccountId
        ? prevAccSnap
        : await trx.get(nextAccRef)
      ...
      trx.update(nextAccRef, {
        balance: fromCents(appliedBalC),
        updatedAt: serverTimestamp()
      })
      ...
      if (patch.account) {
        patch.accountId = patch.account
        delete patch.account
      }
      if (patch.debt) {
        patch.debtId = patch.debt
        delete patch.debt
      }
      if (patch.description) {
        patch.note = patch.description
        delete patch.description
      }
      if (patch.goal) {
        patch.goalId = patch.goal
        delete patch.goal
      }
  ...
  const editTransaction = async (id, payload) => {
    ...
    await runTransaction(db, async trx => {
      ...
      if (prev.accountId === newAccountId) {
        ...
        const [oldAccSnap, newAccSnap] = await Promise.all([
          trx.get(oldAccRef),
          trx.get(newAccRef)
        ])
        ...
        if (appliedBalC < 0) throw new Error('BalanceNegative')
        trx.update(oldAccRef, {
          balance: fromCents(revertedBalC),
          updatedAt: serverTimestamp()
        })
        trx.update(newAccRef, {
          balance: fromCents(appliedBalC),
          updatedAt: serverTimestamp()
        })
      }
  ...
  const subscribeTransactions = (cb, opts = {}) => {
    ...
    return onSnapshot(q, snap => {
      const all = snap.docs.map(d => ({ id: d.id, ...d.data() }))
      cb(all.filter(i => i.isTransfer !== true))
    })
  }
