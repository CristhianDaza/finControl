...
export const useTransfers = () => {
  ...
  const createTransfer = async ({
    ...
    await runTransaction(db, async trx => {
      ...
      trx.update(fromRef, {
        balance: fromCents(nextFromBalC),
        updatedAt: serverTimestamp()
      })
      trx.update(toRef, {
        balance: fromCents(nextToBalC),
        updatedAt: serverTimestamp()
      })
      const base = {
        ownerId: uid,
        ...
        fromAccountId,
        toAccountId,
        amountFrom: fromCents(amountFromC),
        amountTo: fromCents(amountToC),
  ...
  const updateTransfer = async (transferId, patch) => {
    ...
    if (patch.amountFrom != null || patch.date) {
      const validation = validateTransactionPayload({
        type: 'income',
        amount: patch.amountFrom ?? 1,
        accountId: patch.fromAccountId || 'placeholder',
    ...
    await runTransaction(db, async trx => {
      ...
      if (
        (prevOut.ownerId && prevOut.ownerId !== uid) ||
        (prevIn.ownerId && prevIn.ownerId !== uid)
      ...
      const nFromSnap =
        newFromId === prevOut.fromAccountId ? prevFromSnap : await trx.get(nFromRef)
      const nToSnap =
        newToId === prevOut.toAccountId ? prevToSnap : await trx.get(nToRef)
      if (!nFromSnap.exists() || !nToSnap.exists()) throw new Error('AccountNotFound')
      const nFromAcc = nFromSnap.data()
      const nToAcc = nToSnap.data()
      const curFrom =
        patch.currencyFrom || prevOut.currencyFrom || nFromAcc.currency || 'COP'
      const curTo =
        patch.currencyTo || prevOut.currencyTo || nToAcc.currency || curFrom
      const amtFromC = cents(
        patch.amountFrom != null ? patch.amountFrom : prevOut.amountFrom
      )
      ...
      const appliedFromBalC =
        (nFromRef.id === prevFromRef.id ? revertedFromBalC : nFromBalC) - amtFromC
      const appliedToBalC =
        (nToRef.id === prevToRef.id ? revertedToBalC : nToBalC) + amtToC
      if (appliedFromBalC < 0) throw new Error('BalanceNegative')
      trx.update(prevFromRef, {
        balance: fromCents(revertedFromBalC),
        updatedAt: serverTimestamp()
      })
      trx.update(prevToRef, {
        balance: fromCents(revertedToBalC),
        updatedAt: serverTimestamp()
      })
      trx.update(nFromRef, {
        balance: fromCents(appliedFromBalC),
        updatedAt: serverTimestamp()
      })
      trx.update(nToRef, {
        balance: fromCents(appliedToBalC),
        updatedAt: serverTimestamp()
      })
      const basePatch = {
        fromAccountId: newFromId,
        toAccountId: newToId,
        amountFrom: fromCents(amtFromC),
        amountTo: fromCents(amtToC),
        currencyFrom: curFrom,
        currencyTo: curTo,
        rate: fx || null,
        date: patch.date || prevOut.date,
        note: patch.note != null ? patch.note : prevOut.note,
        updatedAt: serverTimestamp()
      }
      trx.update(outRef, {
        ...basePatch,
  ...
  const deleteTransfer = async transferId => {
    ...
    await runTransaction(db, async trx => {
      ...
      const i = inSnap.data()
      if (
        (o.ownerId && o.ownerId !== uid) ||
        (i.ownerId && i.ownerId !== uid)
      ) throw new Error('Unauthorized')
  ...
  const fetchTransferPair = async transferId => {
    ...
    const out = docs.find(out => out.type === 'transfer-out')
    const inn = docs.find(inn => inn.type === 'transfer-in')
    return { out, inn }
  }
