...
export const useGoals = () => {
  const getUserPaths = () => {
    ...
    return { uid, goalsCol: collection(db, 'users', uid, 'goals') }
  }

  const subscribeGoals = (cb, opts = {}) => {
    const { goalsCol } = getUserPaths()
    const clauses = []
    if (opts.paused != null) clauses.push(where('paused', '==', !!opts.paused))
    const q = query(goalsCol, ...clauses, fbOrderBy('createdAt', 'desc'))
  ...
  const createGoal = async (payload) => {
    const { uid, goalsCol } = getUserPaths()
    const ref = doc(goalsCol)
    const data = {
      ownerId: uid,
      name: payload.name,
