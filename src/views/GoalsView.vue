...
const openEdit = g => {
  ...
  form.value = {
    ...
    account: g.accountId || g.account || '',
    dueDate: g.dueDate || '',
    note: g.note || '',
    paused: !!g.paused,
    currency: g.currency || currencies.defaultCurrency.code
  }
...
const save = async () => {
  ...
  if (isEditing.value && editingId.value) {
    await goals.edit(editingId.value, payload)
  } else {
    await goals.add(payload)
  }
  showModal.value = false
  resetForm()
  await goals.loadProgress()
}

const askPauseResume = async g => {
  if (!auth.canWrite) return
  if (g.paused) {
    await goals.resume(g.id)
  } else {
    await goals.pause(g.id)
  }
}
...
<template>
  <section>
      ...
      <table>
        ...
        <tbody>
          <tr v-for="g in goals.items" :key="g.id">
            <td :data-label="t('goals.table.name')">
              {{ g.name }}
            </td>
            <td :data-label="t('goals.table.account')">
              {{ accountNameById[g.accountId] || g.accountId }}
            </td>
            <td :data-label="t('goals.table.target')">
              {{ formatAmount(g.targetAmount, g.currency || currencies.defaultCurrency.code) }}
            </td>
            <td :data-label="t('goals.table.progress')">
              <div class="progress-row">
                ...
                <span class="progress-text">
                  {{ goalProgressPct(g.id) }}%
                </span>
              </div>
            </td>
            <td :data-label="t('goals.table.dueDate')">
              {{ g.dueDate || '-' }}
            </td>
            <td :data-label="t('goals.table.status')">
              <span
                v-if="isCompleted(g.id)"
              ...
              <span
                v-else
                class="badge"
                :class="g.paused ? 'badge-gray' : 'badge-blue'"
              >
                {{ g.paused ? t('goals.paused') : t('goals.active') }}
              </span>
            </td>
    ...
    <FcModal
      ...
      :title-modal="isEditing ? t('goals.edit') : t('goals.add')"
      @accept="save"
