...
const emailError = computed(() => {
  ...
  return /.+@.+\..+/.test(email.value) ? null : t('validation.email-invalid')
})
...
const confirmPasswordError = computed(() => {
  ...
  return confirmPassword.value === password.value ? null : t('errors.signup.password_mismatch')
})

const codeError = computed(() => {
  if (!isSignup.value) return null
  if (!code.value) return t('validation.code-required')
  return null
})
  ...
  <section class="login-wrapper">
    ...
    >
      <div class="login-inner">
        <img
          src="@/assets/images/logo-fin-control.png"
          alt="Logo"
          class="login-logo"
        />
        <h1 id="login-title">
        ...
        <form @submit.prevent="submit" novalidate class="login-form">
          ...
          <label class="field">
            <span>
              {{ isSignup ? t('auth.signup.password') : t('auth.login.password') }}
            </span>
            <input
              class="input"
              type="password"
          ...
          <button
            class="button btn-primary full-width"
            :disabled="auth.status === 'loading'"
            :aria-disabled="auth.status==='loading'"
          >
            {{
              auth.status === 'loading'
                ? (isSignup ? t('auth.signup.submitting') : t('auth.login.submitting'))
                : (isSignup ? t('auth.signup.submit') : t('auth.login.submit'))
            }}
          </button>
          <button
            class="button button-secondary toggle-btn"
            type="button"
            @click="toggle"
          >
            {{
              isSignup
                ? t('auth.signup.toggleLogin')
                : t('auth.signup.toggleSignup')
            }}
          </button>
        </form>
      </div>
...
.input:hover {
  background-color: color-mix(in srgb, var(--secondary-color) 92%, var(--accent-color));
}
...
@media (min-width: 960px) {
  ...
  .login-form > .field:nth-of-type(n+3) {
    grid-column: span 1;
  }
  .login-form > .error,
  .login-form > p.error {
