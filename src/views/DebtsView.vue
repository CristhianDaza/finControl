...
const openCreate = () => { if (!auth.canWrite) return; editing.value = null; modalTitle.value = t('debts.addTitle'); showModal.value = true }
const openEdit = (item) => { if (!auth.canWrite) return; editing.value = item; modalTitle.value = t('debts.editTitle'); showModal.value = true }
const askRemove = (id) => { if (!auth.canWrite) return; toDeleteId.value = id; confirmOpen.value = true }

const ensurePaymentsLoaded = async (debtId) => { if (paymentsByDebt.value[debtId]) return; paymentsByDebt.value[debtId] = await fetchTransactions({ type: 'debtPayment', debtId, orderBy: [{ field: 'date', dir: 'desc' }, { field: 'createdAt', dir: 'desc' }] }) }
const toggleOpen = async (debtId) => { opened.value[debtId] = !opened.value[debtId]; if (opened.value[debtId]) await ensurePaymentsLoaded(debtId) }

const onSave = async (payload) => { if (!auth.canWrite) return; busy.value = true; try { if (payload.id) { await deb.update(payload.id, { name: payload.name, dueDate: payload.dueDate }) } else { await deb.create({ name: payload.name, amount: payload.amount, dueDate: payload.dueDate, currency: payload.currency }) } showModal.value = false; editing.value = null } finally { busy.value = false } }
const onConfirmDelete = async () => { if (!auth.canWrite) return; if (!toDeleteId.value) return; busy.value = true; try { await deb.remove(toDeleteId.value) } catch (e) { if (e && (e.code === 'debt/has-payments' || /pagos/i.test(e.message))) blockOpen.value = true } finally { busy.value = false; toDeleteId.value = null; confirmOpen.value = false } }
  ...
  <section>
    <div class="card page-header">
      ...
      <div class="page-actions">
        <button class="button" @click="openCreate" :disabled="busy || isLoading || !auth.canWrite" :aria-disabled="!auth.canWrite">{{ t('debts.addButton') }}</button>
      </div>
    </div>

    <DebtsModalComponent :show-modal-debts="showModal" :initial="editing || undefined" :title="modalTitle" @save="onSave" @update:showModal="showModal = $event" />
          ...
          <template v-for="d in rows" :key="d.id">
            <tr>
              ...
              <td :data-label="t('transactions.table.actions')">
                <div class="actions">
                  <button class="button button-edit" :aria-label="t('common.edit')" :title="t('common.edit')" @click="openEdit(d)" :disabled="!auth.canWrite" :aria-disabled="!auth.canWrite"><svg class="icon-edit" v-html="EditIcon"></svg></button>
                  <button class="button button-delete" :aria-label="t('common.delete')" :title="t('common.delete')" @click="askRemove(d.id)" :disabled="!auth.canWrite" :aria-disabled="!auth.canWrite"><svg class="icon-delete" v-html="DeleteIcon"></svg></button>
                </div>
              </td>
            </tr>
            <tr v-if="opened[d.id]">
              <td :colspan="6" class="details-cell">
                <div class="payments-block">
                  ...
                  <ul v-else class="payments-list">
                    <li v-for="p in paymentsByDebt[d.id]" :key="p.id"><span>📓 {{ formatDate(p.date) }}</span><span>{{ formatAmount(p.amount, p.currency || d.currency || currencies.defaultCurrency.code) }}</span></li>
                  </ul>
                </div>
              </td>
            </tr>
    ...
    <FcModal :show-modal="confirmOpen" @update:showModal="confirmOpen = $event" @accept="onConfirmDelete" @cancel-modal="confirmOpen = false" :title-modal="t('debts.confirmDelete.title')" :accept-disabled="!auth.canWrite">
      <p>{{ t('debts.confirmDelete.title') }}: {{ t('common.delete') }}?</p>
    </FcModal>
    <FcModal :show-modal="blockOpen" @update:showModal="blockOpen = $event" @accept="blockOpen = false" @cancel-modal="blockOpen = false" :title-modal="t('debts.confirmDelete.title')">
      <p>{{ t('debts.confirmDelete.message') }}</p>
      <div style="display:flex;gap:.5rem;justify-content:flex-end"><button class="button" @click="() => { blockOpen = false; router.push({ name: 'transactions' }) }">{{ t('debts.confirmDelete.cta') }}</button></div>
    </FcModal>
  </section>
</template>
