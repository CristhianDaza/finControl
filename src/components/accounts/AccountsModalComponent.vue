...
const handleAccept = () => {
  ...
  const payload = {
    id: account.value.id,
    name: String(account.value.name || '').trim(),
    balance: Number(account.value.balance || 0),
    currency: account.value.currency || currencies.defaultCurrency.code
  }
  emit('save', payload)
...
watch(() => props.showModalAccounts, v => {
  showModal.value = v
})
