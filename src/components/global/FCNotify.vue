...
const onPointerDown = (e, id) => {
  ...
  drags.value[id] = { startX: e.clientX, startY: e.clientY, x: 0, active: true }
...
.toast.success { box-shadow: inset 3px 0 0 0 var(--success-color), 0 8px 22px rgba(0,0,0,.28); }
.toast.error   { box-shadow: inset 3px 0 0 0 var(--error-color),   0 8px 22px rgba(0,0,0,.28); }
.toast.info    { box-shadow: inset 3px 0 0 0 var(--info-color),    0 8px 22px rgba(0,0,0,.28); }
.toast.warn    { box-shadow: inset 3px 0 0 0 var(--warning-color), 0 8px 22px rgba(0,0,0,.28); }
...
.toast.success { color: var(--success-color);   --icon: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' fill='%23000' viewBox='0 0 24 24'><path d='M9 16.2 4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4z'/></svg>"); }
.toast.error   { color: var(--error-color);     --icon: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' fill='%23000' viewBox='0 0 24 24'><path d='M12 2 2 22h20L12 2zm1 15h-2v-2h2v2zm0-4h-2V9h2v4z'/></svg>"); }
.toast.info    { color: var(--info-color);      --icon: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' fill='%23000' viewBox='0 0 24 24'><path d='M11 17h2v-6h-2v6zm0-8h2V7h-2v2z'/></svg>"); }
.toast.warn    { color: var(--warning-color);   --icon: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' fill='%23000' viewBox='0 0 24 24'><path d='M1 21h22L12 2 1 21zm12-3h-2v-2h2v2zm0-4h-2V9h2v5z'/></svg>"); }
...
.toast-close:hover { background: color-mix(in oklab, var(--secondary-color) 72%, transparent); color: var(--text-color); }
...
.toast-close .x {
  inline-size: 14px; block-size: 14px; background: currentColor;
...
@keyframes lifeline { from { transform: scaleX(1); } to { transform: scaleX(0); } }

.toast-enter-active, .toast-leave-active {
  transition: opacity .18s ease, transform .18s ease, filter .18s ease;
}
.toast-enter-from { opacity: 0; transform: translateY(-8px) scale(.98); filter: blur(.5px); }
.toast-leave-to   { opacity: 0; transform: translateY( 8px) scale(.98); filter: blur(.6px); }

@media (prefers-reduced-motion: reduce) {
  .toast-enter-active, .toast-leave-active { transition: none; }
  .lifebar::before { animation: none; }
}
</style>
