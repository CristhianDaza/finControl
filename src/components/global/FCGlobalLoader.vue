...
.fc-gl-loader__backdrop {
  ...
  background:
    radial-gradient(
      ...
      color-mix(in srgb, var(--overlay-50) 85%, transparent)
    ),
    var(--overlay-50);
...
.fc-gl-loader__content {
  ...
  border-radius: 20px;
  background: linear-gradient(145deg, var(--secondary-color) 0%, var(--primary-color) 60%);
  box-shadow: 0 8px 32px -8px var(--shadow-elev-4), 0 0 0 1px rgba(255,255,255,0.04);
  pointer-events: auto;
}
...
.fc-gl-loader__spinner {
  ...
  background: radial-gradient(
    ...
    color-mix(in srgb, var(--secondary-color) 92%, transparent)
  );
  backdrop-filter: blur(2px);
}

.fc-gl-loader__spinner::before {
  content: "";
  position: absolute;
  inset: -6px;
  border-radius: 50%;
...
.fc-gl-loader__spinner .ring {
  position: absolute;
  inset: 0;
  border-radius: 50%;
  border: 4px solid transparent;
  animation: fc-gl-spin 1.2s linear infinite;
  mix-blend-mode: normal;
}

.fc-gl-loader__spinner .ring:nth-child(1) {
  border-top-color: var(--accent-color);
  border-bottom-color: var(--accent-color);
  filter: drop-shadow(0 0 4px var(--accent-color));
}

.fc-gl-loader__spinner .ring:nth-child(2) {
...
.fc-gl-loader__spinner .ring:nth-child(3) {
  border-top-color: var(--debt-color);
  border-left-color: var(--debt-color);
  animation-duration: 1.1s;
  filter: drop-shadow(0 0 4px var(--debt-color));
}

@keyframes fc-gl-spin {
  to {
    transform: rotate(360deg);
  }
}

.fc-gl-loader-fade-enter-active,
.fc-gl-loader-fade-leave-active {
  transition: opacity .35s ease, transform .4s ease;
}

.fc-gl-loader-fade-enter-from,
.fc-gl-loader-fade-leave-to {
  opacity: 0;
  transform: scale(.98);
}
</style>
